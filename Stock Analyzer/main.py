...
def Get_Data(i):
    ...
    data[i] = stock_info.get_data(
        f"{i}", start_date='01-01-2013',  end_date='11-19-2022')
...
def Analyze_Results():
    ...
    for index, row in aggr_results.iterrows():
        if row['Aggr Condition'] == "Buy" and not is_buying:
            ...
            if row['Aggr Condition'] == "Sell":
                ...
                print(
                    f"{row['Aggr Condition']} at {round(float(row['Close']),2)} on {row['Index']} with {row['Volume']}\n")
                lap_gain = round((lap_revenue + lap_total) /
                                 abs(lap_total)*100, 2)
                sum_laps += lap_gain
                stop_date = row['Index']
                # try:
    ...
    print(
        f"Total Gain to Date: {round((running_revenue + running_total)/abs(running_total)*100,2)}")
